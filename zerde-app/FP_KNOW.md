...
5. **Триязычие (KZ / RU / EN) и 3 копии в БД:** При генерации курсов в БД сохраняются 3 языковые копии задачи. При смене языка подтягивается соответствующая копия без искажения формул и педагогических терминов. Непереводимые бренды и термины (`ELO`, `Aga`, `Zerde`, `ZVDSL+`, `Thought-Forks`, `Eureka`, `Q-Matrix`, `CDM`) имеют защищенный статус.
